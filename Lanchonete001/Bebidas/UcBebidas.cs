...
{
    public partial class UcBebidas : UserControl
    {
        private const string PlaceholderBusca = "Buscar bebida...";
        private static readonly string[] Categorias = { "Refrigerantes", "Sucos", "Águas", "Milk-shakes" };

        private List<ItemCardapio> listaBebidas;
        private BindingSource bindingBebidas;
        ...
        {
            listaBebidas = CardapioRepositorio.ObterBebidas();

            if (bindingBebidas == null)
            {
                bindingBebidas = new BindingSource { DataSource = listaBebidas };
                dataGridView1.DataSource = bindingBebidas;
            }
        ...
        private void AplicarFiltro()
        {
            ...
            var filtrados = listaBebidas.Where(i =>
            ...
            bindingBebidas.DataSource = filtrados;
        ...
        {
            var frm = new FrmNovoItemCardapio(TipoItemCardapio.Bebida, item);
        ...
        {
            int total = listaBebidas.Count;
            int indisponiveis = listaBebidas.Count(i => !i.DisponivelParaVenda);
            decimal margemMedia = total > 0 ? listaBebidas.Average(i => i.PercentualMargem) : 0;
        ...
        private void btnNovoBebida_Click(object sender, EventArgs e)
        {
            var frm = new FrmNovoItemCardapio(TipoItemCardapio.Bebida);
            if (frm.ShowDialog() == DialogResult.OK)
            {
                CardapioRepositorio.Adicionar(frm.ItemCriado);
                listaBebidas.Add(frm.ItemCriado);
                AplicarFiltro();
                AtualizarCardsResumo();
            }
        }
